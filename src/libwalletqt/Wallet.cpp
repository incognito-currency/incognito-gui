...
class WalletListenerImpl : public  Incognito::WalletListener
{
public:
...
{
    QFuture<Incognito::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Incognito::Wallet::connected);
    QFutureWatcher<Incognito::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Incognito::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Incognito::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Incognito::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Incognito::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Incognito::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Incognito::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Incognito::optional<uint64_t>(), mixin_count,
                static_cast<Incognito::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Incognito::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Incognito::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Incognito::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/incognito.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/incognito-wallet-gui.log";
}

Wallet::Wallet(Incognito::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Incognito::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
