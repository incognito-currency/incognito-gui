...
{
    // prepare command line arguments and pass to incognitod
    ...
    qDebug() << "starting incognitod " + m_incognitod;
    ...
    // Start incognitod
    bool started = m_daemon->startDetached(m_incognitod, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM incognitod.exe");
#else
                QProcess::execute("pkill incognitod");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./incognitod status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_incognitod, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to incognitod
#ifdef Q_OS_WIN
    m_incognitod = QApplication::applicationDirPath() + "/incognitod.exe";
#elif defined(Q_OS_UNIX)
    m_incognitod = QApplication::applicationDirPath() + "/incognitod";
#endif

    if (m_incognitod.length() == 0) {
