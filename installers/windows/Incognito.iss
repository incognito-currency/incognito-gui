; Incognito Helium Hydra GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Incognito Project
; See LICENSE

[Setup]
AppName=Incognito GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Incognito GUI Wallet
DefaultGroupName=Incognito GUI Wallet
UninstallDisplayIcon={app}\incognito-wallet-gui.exe
...
; As of version 0.11.1.0, the Incognito .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Incognito.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Incognito GUI wallet
Source: "bin\incognito-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Incognito GUI wallet log file
...
Source: "incognito-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Incognito CLI wallet
Source: "bin\incognito-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Incognito wallet RPC interface implementation
Source: "bin\incognito-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Incognito daemon
Source: "bin\incognitod.exe"; DestDir: "{app}"; Flags: ignoreversion

; Incognito daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "incognito-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Incognito blockchain utilities
Source: "bin\incognito-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\incognito-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\incognito-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\incognito-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\incognito');
  s := 'The default folder to store the Incognito blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\incognito.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "incognito-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Incognito daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\incognitod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\incognito-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Incognito GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\incognito-wallet-gui.exe"
...
Name: "{group}\Utilities\Incognito Daemon"; Filename: "{app}\incognitod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\incognito-wallet-cli.exe"; WorkingDir: "{userdocs}\Incognito\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Incognito\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\incognito-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\incognito-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM incognitod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\incognito-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\incognito-currency"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\incognito-currency\incognito-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\incognito-currency\incognito-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
