...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/incognito-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    INCOGNITOD_EXEC=incognitod.exe
fi
...
get_tag
popd
echo "var GUI_INCOGNITO_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../incognito-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy incognitod to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$INCOGNITO_DIR/bin/$INCOGNITOD_EXEC $BIN_PATH
fi
